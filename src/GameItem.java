...
public class GameItem {
    ...
    public static GameItem[] readItems(File file) {
        List<GameItem> items = new ArrayList<>();  // List to store all parsed GameItems
        ItemProps currentProps = new ItemProps();  // Temporary holder for the current item's properties

        try (var reader = new BufferedReader(new FileReader(file))) {
            for (String line = reader.readLine(); line != null; line = reader.readLine()) {
                // Parse each line, update currentProps, or store a GameItem
                parseItemLine(line, items, currentProps);
            }
            // At end of file, there may be one last item not yet added
        ...
        return items.toArray(new GameItem[0]); // Convert the List into an array and return it
    }
    ...
    private static void parseItemLine(String line, List<GameItem> items, ItemProps props) {
        if (line.startsWith("[") && line.endsWith("]")) {
            // If props already has a name, the previous item is complete → save it to the list
            if (props.name != null) {
                items.add(new GameItem(props.name, props.value, props.weight, props.attackBonus,
                                       props.agilityBonus, props.defenseBonus));
            }
            // Extract the new item name (remove the square brackets)
            props.name = line.substring(1, line.length()-1);
            // Reset all numeric stats to defaults (0)
            props.resetStats();
        } else if (line.contains("=")) {
            // If it's a key=value property line (e.g., Weight=15), Split into 2 parts by "="
